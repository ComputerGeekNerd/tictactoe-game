...
class App extends React.Component {
	...
	handleClick(row, col) {
		...
		const updateSquare = [...history[history.length - 1].squares].map(
			(row) => [...row]
		);
		if (
			this.calculateIfWinner(updateSquare).winner ||
	...
	handleMoveTo(step) {
		...
		history = history
			.slice(0, step + 1)
			.map((sqObj) => ({
				squares: [...sqObj.squares.map((row) => [...row])]
			}));
		this.setState(function (prevState) {
			return {
				history: [...history],
	...
	render() {
      ...
      :'';
		...
		return (
			<div className="min-h-screen text-center screen-bg">
				...
				<div className="game flex flex-col items-center md:flex md:flex-row md:justify-center md:items-start pt-6">
					...
					<div className="step md:ml-16">
						<div
							className={
								winner
									? 'result bg-green-400 inline-block font-bold uppercase my-4  text-md p-4'
									: 'result mt-8 mb-4 md:mt-0 md:mb-2 inline-block'
							}
						>
							{status}
						</div>
        ...
        <h5 className="text-l text-center tracking-tighter text-red-400 pt-4 mt-10">
					Created By: Raksha Aasish
				</h5>
			</div>
		);
	}
